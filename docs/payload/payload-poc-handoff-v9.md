...
> **Date:** June 2026 (updated post–AI Agent direction confirmed)
> **Summary:** Wired the Apirtayo Next.js frontend to an external Payload CMS for dynamic homepage content, replacing hardcoded data in four homepage sections. Both repos are deployed to Vercel. A media filename sanitization fix was applied. A standalone team-facing explainer (presentation) was completed. The AI Agent client portal direction has been confirmed by Sir Jeff — tenant-based portal, implemented in `apir-tayo`, using a separate `PortalClients` collection in Payload. Phased implementation plan established. Next session's focus: Phase 1 — PortalClients collection + portal login page.
...
- **Media filename sanitization fix applied** — certain filenames were being rejected by the Supabase S3 bucket due to invalid key characters; a cleaning step was added before upload to strip/replace disallowed characters before the file hits storage
...
- **Standalone team-facing presentation completed** — a self-contained explainer ("What is Payload, why we use it, how it works, how the frontend connects") prepared for the team presentation; separate from this technical handoff
- **AI Agent client portal direction confirmed** — Sir Jeff confirmed the portal will be tenant-based; full phased implementation plan established (see §7.5)
...
## 7.5 AI Agent — Client Portal (Workstream A)

Direction is now confirmed. This is the focus for the next working session.

### Confirmed decisions
- **Sir Jeff confirmed:** the portal will be **tenant-based** — one portal per tenant, agent scoped to everything under that tenant
- **Auth:** real login, wired to Payload + MongoDB — no stubbed/simulated auth even for POC
- **User creation:** your team manually creates `PortalClient` records in Payload admin and hands credentials + URL to the client — no self-signup flow needed
- **Access point:** direct URL (`/portal/login`) for now — no public link from the main site unless requested later
- **Portal location:** `/portal` route group inside `apir-tayo` — no new repository needed for POC stage
- **Auth collection:** separate `PortalClients` collection in `payload-poc` (not mixed into the admin `Users` collection) — cleaner access control separation

### Why a separate PortalClients collection (not Users)
Payload supports multiple auth-enabled collections. Keeping `PortalClients` separate from `Users` means:
- Client users can never accidentally get Payload admin access
- Access control rules are simpler to reason about per collection
- The two user types are cleanly distinct at the data level

`PortalClients` schema (to build in Phase 1):
```
PortalClients
- email         (auth field, built-in)
- password      (auth field, Payload handles hashing)
- tenant        (relationship → Tenants collection, required)
```

### Login flow (once built)
```
Client visits /portal/login in apir-tayo
  ↓ submits email + password
apir-tayo calls POST /api/portal-clients/login on payload-poc
  ↓ Payload validates credentials, returns JWT
apir-tayo stores token in session
  ↓ resolves tenantId from the logged-in PortalClient
/portal/chat is now accessible, all agent calls scoped to that tenantId
```

### Phased implementation plan

**Phase 1 — Portal Auth** ← *next session starts here*
- Add `PortalClients` collection in `payload-poc` (email, password, tenant relationship)
- Build `/portal/login` page in `apir-tayo`
- On login, call `POST /api/portal-clients/login`, store token in session
- Resolve and store `tenantId` from the logged-in user's record

**Phase 2 — Portal Chat UI**
- Build `/portal/chat` page in `apir-tayo` (protected route — redirect to login if no session)
- Simple chat interface: input box, message history display
- Sends message + `tenantId` to the agent API route

**Phase 3 — Wire Agent to Tenant**
- Agent route reads `tenantId` from the incoming request (set by the portal session)
- Fix the existing `// TODO(Workstream A)` in `src/app/api/agent/route.ts` — currently resolves a tenant slug, needs to resolve a **Site** slug using `resolveSite.ts` and derive tenant from `site.tenant`
- All Payload queries and writes scoped to that tenant
- Text field edits only to start (safest, most predictable)

**Phase 4 — Guardrails**
- Confirmation step before applying changes ("Do you want to update the hero headline to X?")
- Basic audit log of agent actions per tenant
- Rollback/undo capability (future consideration)

### Current state of the agent POC
- Lives at `src/app/api/agent/route.ts` in `payload-poc`
- Plain-language input → **DeepSeek API** parses intent → scoped Payload REST API calls
- Core flow and isolation are UAT-confirmed at POC level
- **Status: benched pending Phase 1–2 portal work** — agent route exists and works, but has no auth layer or portal UI to connect to yet

### Known remaining gaps in the agent (still open)
- Updating Lexical rich text fields (hero text, etc.) isn't done — these use a specific JSON structure, not plain strings; agent content-update logic needs to produce that structure correctly
- `AGENT_EMAIL` service account role restrictions in Payload aren't locked down yet — currently broader than strictly needed
- On-demand revalidation (deprioritized in §7) becomes relevant once agent edits are live — clients will expect changes to appear near-instantly, not after a 60s ISR window. Worth revisiting before Phase 3 ships.
