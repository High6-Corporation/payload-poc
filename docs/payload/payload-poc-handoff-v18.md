...
> **Date:** June 27, 2026
> **Version:** v18
> **Summary:** Documentation-only session. No code written. Roadmap Word document generated (`Payload-CMS-Future-Roadmap.docx`) covering Phases 15–21 with implementation sequencing, effort estimates, and dependencies. Forms source audit completed — critical finding: `forms` and `form-submissions` are not tenant-scoped (gap documented in §2 and §10). **Next session:** fix `getTenantSiteId()` in `src/app/api/agent/actions/shared.ts` to support multiple sites per tenant so the Website Team can begin testing with a second site.
...
Tenant ("High6 Corporation")
  ...
  └── Site ("future-project-2")   ← currently blocked by getTenantSiteId() assumption
        └── ...
...
- Roadmap document generated covering Phases 15–21 (this session)
...
> ⚠️ `CLEANTALK_API_KEY` currently in apir-tayo `.env` — **move to payload-poc** as part of Phase 15. It must never be `NEXT_PUBLIC_`.
...
| `CLEANTALK_API_KEY` | Pending (Phase 15) | Move here from apir-tayo |
...
| `src/app/api/agent/route.ts` | Orchestration shell — parses body, gets token, calls DeepSeek, delegates to handlers |
...
| `src/app/api/agent/shared.ts` | `awaiting_value` guard + `awaiting_fields` state machine + `buildCreateProposal` + `buildFieldPrompt` |
| `src/app/api/agent/audit.ts` | `writeAuditLog()` |
| `src/app/api/agent/actions/shared.ts` | `patchRecord()`, `patchWithRetry()`, `postRecord()`, `createWithRetry()`, **`getTenantSiteId()`** ← fix target |
...
| `src/app/api/agent/actions/pricing.ts` | `handlePricing()` — features→items transform, price string→float coercion |
...
### Phase 15 — `getTenantSiteId()` Multi-Site Fix ← **NEXT SESSION**

**File:** `src/app/api/agent/actions/shared.ts`

**Current behavior:** `getTenantSiteId()` assumes one site per tenant. It fetches the sites collection filtered by tenant and returns the first result's ID. This assumption is documented in the file itself. It breaks silently for any tenant with more than one site — the wrong site's content is used.

**Why this is next:** The Website Team needs to test with a second site under the High6 Corporation tenant. Without this fix, the agent routes all operations to the wrong site the moment a second site exists.

**What the fix looks like:**
- `getTenantSiteId()` needs to accept a `siteId` or `siteSlug` parameter rather than assuming the first result.
- The `siteId` is already available at the agent route level — it should be threaded through the call chain from `route.ts` → action handlers → `getTenantSiteId()`.
- `portal/chat/page.tsx` (apir-tayo) already resolves and passes the site slug to ChatWindow. Confirm that the `siteId` is being forwarded in `POST /api/portal/agent` — check `app/api/portal/agent/route.ts`.
- Verify: after the fix, two sites under the same tenant operate independently. An agent session on Site A cannot read or write Site B's records.

**Scope:** `payload-poc` only. No frontend changes expected unless `siteId` is missing from the proxy request body.
...
|---|---|---|
| `getTenantSiteId()` assumes one site per tenant | Breaks agent for any multi-site tenant. Website Team is blocked. | **Next session (Phase 15)** |
