...
> **Date:** June 29, 2026
> **Version:** v20
> **Summary:** Phase 15 verification pending (two-site isolation test not yet run). `feat/payload-cms` successfully promoted to production on Hostinger VPS via `feat/payload-cms` → `staging` → `master` merge sequence. Deployment verified end-to-end — dynamic content (FAQs, Testimonials, Portfolio Items, Pricing Plans, SiteSettings), AI Agent portal add/delete/update all confirmed working in production. MongoDB Atlas migration to a new account is noted but not yet executed. **Next session:** Phase 16 — Forms tenant scoping + anti-spam (CleanTalk).
...
### Cumulative achievements (Phases 1–15 + Production Promotion)
...
> ⚠️ **`forms` and `form-submissions` are not tenant-scoped.** The multi-tenant plugin does not cover these collections. This is the primary focus of Phase 16 (next session).
...
| `CLEANTALK_API_KEY` | Server-only | VPS `.env` only | ⚠️ Move to payload-poc in Phase 16 |
...
> ⚠️ `CLEANTALK_API_KEY` is currently in apir-tayo `.env` — **move to payload-poc** as part of Phase 16. It must never be `NEXT_PUBLIC_`.
...
| `SMTP2GO_*` | Pending (Phase 17) | SMTP credentials — host, port, username, password, from address |
...
| Rotate exposed credentials | 🔴 High | `.env` was committed to `master` before repo went public. Credentials must be rotated. Still outstanding. |
...
### Phase 16 — Forms: Tenant Scoping + Anti-Spam (CleanTalk) ← **NEXT SESSION**
...
6. `@payloadcms/plugin-form-builder` — Payment: disabled. Custom `confirmationMessage` editor. ⚠️ `forms` and `form-submissions` not yet tenant-scoped (Phase 16).
...
| `forms` / `form-submissions` not tenant-scoped | Cross-tenant form access possible. No submission provenance. | **Phase 16 — next session** |
